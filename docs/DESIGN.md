...
| 캐시/Presence | Redis | TTL key, set, Cache Aside |
...
`clientMessageId`는 ACK/NACK correlation 용도다. 현재 DB 저장 멱등성은 Kafka event의 `messageKey` 기준이며, 클라이언트 재시도까지 DB-level idempotency로 막으려면 `(senderId, clientMessageId)` unique constraint가 추가로 필요하다. 현재 schema와 entity에는 `clientMessageId`를 저장하지 않는다.
