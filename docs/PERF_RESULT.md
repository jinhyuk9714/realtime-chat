...
- **제한된 메시지 smoke:** 테스트는 메시지를 전송하고 `MESSAGE` frame 수신 여부를 카운트하지만, 모든 전송 메시지의 수신 완료를 claim하지 않으며 검증하지 않는다.
...
**결과:** local smoke 실행 확인, 10-room/50-user mixed HTTP probe repeat3는 시나리오 검증
...
2026-05-23에는 `scripts/ws-delivery-runner.mjs --mixed-http-probes true`로 10 rooms / room당 50 users
local repeat3를 실행했다. 이 결과는 k6 RPS benchmark가 아니라, 같은 artifact 안에 room list, message
history, read receipt HTTP probe를 남긴 receiver matrix 시나리오 검증이다.

| run | accepted sends | expected | unique | missing | duplicate | receiver p95 | HTTP failed |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | 100 | 4,900 | 4,900 | 0 | 0 | 20ms | 0 / 30 |
| 2 | 100 | 4,900 | 4,900 | 0 | 0 | 18ms | 0 / 30 |
| 3 | 100 | 4,900 | 4,900 | 0 | 0 | 20ms | 0 / 30 |

세 run 모두 `scripts/validate-delivery-evidence.mjs`를 통과했다. 요약은
...
분리한다. 이 결과도 local 단일 앱 시나리오 검증이며 claim boundary: production, multi-instance, 1,000-session mixed
benchmark나 cache hit ratio claim으로 확장하지 않는다.
...
recipient 기준 local receiver matrix 반복 검증으로만 사용하고, production mixed traffic p95나
room-global ordering 수치로 해석하지 않습니다.
...
fan-out 검증으로만 해석합니다. Claim boundary: production mixed traffic p95와 production benchmark는 별도 측정 대상입니다.
...
| Mixed HTTP probe receiver matrix | - | 10-room/50-user local repeat3 모두 completeness 100%, receiver p95 18-20ms, HTTP failed 0/90 | local 단일 앱 시나리오 검증, production benchmark 아님 |
| Mixed chat k6 scenario | - | local smoke 실행 확인 | RPS benchmark는 추가 측정 예정 |
...
**한계:** 실제 트래픽은 읽기(채팅방 목록, 메시지 이력)와 쓰기(메시지 전송, 읽음 처리)가 혼합된다. local 시나리오 검증은 있지만 claim boundary: production send-to-receive end-to-end latency, 수신 completeness, 메시지 순서 정확도 수치는 아직 없다.

**개선 방향:** `k6/mixed-chat-test.js`의 local smoke와 10-room/50-user mixed HTTP probe repeat3는
통과했지만, claim boundary: production RPS/p95와 cache hit ratio를 공개 benchmark로 올리려면 더 큰 환경과 반복 실행을
별도로 기록해야 한다.
