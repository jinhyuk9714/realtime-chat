...
| 구분 | 항목 | 결과 / 상태 | 조건 |
|---|---|---|---|
| 측정 완료 | 채팅방 조회 API 최적화 | 937 -> 1,598 RPS | k6 200 VU / 50s / local Docker 기준 |
| 측정 완료 | p95 응답 시간 | 212.85ms -> 149.22ms | `GET /api/rooms` 중심 REST 조회 테스트 |
| 측정 완료 | N+1 제거 | 방 N개 기준 2N+1회 쿼리 -> 1회 쿼리 | JPQL Projection |
| 시나리오 검증 | WebSocket 연결 smoke | 2대 합산 1,158 sessions, 연결 체크 성공률 100% | 연결 안정성만 확인, send-to-receive latency 측정 아님 |
| 시나리오 검증 | SUBSCRIBE 권한 검증 | 비멤버 room topic 구독 거부 | Unit + STOMP integration 테스트 |
| 시나리오 검증 | Redis global SEND rate limit | user-level fixed-window 제한, Redis 장애 시 fail-closed | Unit 테스트 |
| 시나리오 검증 | Kafka publish ACK/NACK | user destination으로 ACK/NACK 전달 | Kafka publish accepted/failed 기준 |
| 시나리오 검증 | DB persisted ACK | DB 저장 완료 후 `/user/queue/messages/persisted` 전달 | Unit + STOMP integration 테스트 |
| 시나리오 검증 | Receiver matrix low-rate 검산 | accepted/status 분모와 expected/actual delivery matrix 계산 경로 확인 | raw snapshot은 `docs/evidence`, 반복 benchmark 아님 |
| 시나리오 검증 | Receiver matrix by-room guard | `summary.byRoom`으로 room별 denominator와 cross-room unexpected delivery 검산 | deterministic fixture, multi-room benchmark 아님 |
| 시나리오 검증 | Delivery evidence validator manifest smoke | 2-user local artifact의 manifest/raw JSONL/summary/byRoom 검산 통과 | [`docs/evidence/DELIVERY_EVIDENCE_VALIDATOR_MANIFEST_2026-05-22.md`](docs/evidence/DELIVERY_EVIDENCE_VALIDATOR_MANIFEST_2026-05-22.md), benchmark 아님 |
| 시나리오 검증 | Receiver matrix 50-user local repeat3 | 3회 모두 expected 4,900 / unique 4,900 / missing 0 / duplicate 0, p95 23-38ms | local Docker Compose 반복 실행, 운영 성능 claim이나 500/1,000 session benchmark 아님 |
| 시나리오 검증 | Receiver matrix 500-user local repeat3 | 3회 모두 expected 49,900 / unique 49,900 / missing 0 / duplicate 0, p95 37-47ms, p99 46-233ms | local Docker Compose 반복 실행, 1,000 session benchmark나 운영 성능 claim 아님 |
| 시나리오 검증 | Reconnect message sync | `afterMessageId` 이후 누락 메시지 REST 동기화 | Integration 테스트 |
| 시나리오 검증 | client retry idempotency | `(senderId, clientMessageId)` 기준 중복 DB 저장 방지 | Unit + integration 테스트 |
| 시나리오 검증 | DLT manual replay | replay 후 `messageKey` 기준 중복 저장 방지 | Testcontainers 기반 통합 테스트 |
| 시나리오 검증 | room 단위 ordering | 동일 room 메시지의 partition / offset 순서 검증 | 전역 순서 보장 아님 |
| 시나리오 검증 | read receipt 정합성 | sender 제외, joinedAt 이전 메시지 제외 | Service / integration 테스트 |
| 시나리오 검증 | multi-session presence | 마지막 session 종료 시에만 offline | Redis session key / set |
| 시나리오 검증 | selective cache eviction | room member의 `rooms::{userId}`만 evict | 관계없는 사용자 cache 유지 |
| 시나리오 검증 | mixed chat local smoke | ACK 100%, NACK 0%, mixed error 0% | `SMOKE=1`, 1 VU, benchmark 아님 |
| 추가 측정 예정 | mixed traffic p95 latency | benchmark 미측정 | `k6/mixed-chat-test.js` smoke와 분리 |
| 추가 측정 예정 | 1,000 session send-to-receive latency | benchmark 미측정 | 50-user repeat3와 500-user repeat3는 [`docs/WEBSOCKET_MEASUREMENT.md`](docs/WEBSOCKET_MEASUREMENT.md)에 시나리오 검증으로 분리 |
| 추가 측정 예정 | 1,000 session delivery completeness | benchmark 미측정 | local receiver matrix repeat3는 운영 성능 claim으로 확장하지 않음 |
...
| send-to-receive p95 latency | local smoke는 self echo 기준으로만 확인했으며, receiver 기준 p95/completeness는 추가 측정 예정 |
...
| 아직 측정하지 않은 것 | 상태 |
|---|---|
| send-to-receive p95 latency | 50-user repeat3 / 500-user repeat3 local scenario는 시나리오 검증, 1,000 session 공개 benchmark는 추가 측정 예정 |
| 모든 전송 메시지의 delivery completeness | 50-user repeat3 / 500-user repeat3 local scenario는 시나리오 검증, 1,000 session 공개 benchmark는 추가 측정 예정 |
| room별 수신 순서 정확도 성능 결과 | 추가 측정 예정 |
| mixed traffic 기준 RPS / p95 | smoke 실행 확인, benchmark는 추가 측정 예정 |
...
| 성능 결과 | local smoke 실행 확인, benchmark는 추가 측정 예정 |
